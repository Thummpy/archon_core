...
- **Read-write volume mounts for workflows, commands, and config** — All three are mounted read-write. Workflows and commands are rw so the workflow builder UI can write new definitions back to the repo. Config is rw because Archon's setup wizard writes credentials to `config.yaml` at runtime, and the container entrypoint (`chown -Rh appuser:appuser /.archon`) exits fatally if any `:ro` target exists inside `/.archon`. Do not mount anything inside `/.archon` with `:ro`. The git repo remains the source of truth — `git diff` reveals any runtime writes before you commit.
